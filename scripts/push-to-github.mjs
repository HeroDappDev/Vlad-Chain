...
try {
  ...
  console.log("GitHub repository is empty; creating its initial commit.");
}

// Put text content directly in one tree request. Binary files still need blob
// objects because the tree API accepts inline UTF-8 text only. This keeps a
// full snapshot to a handful of requests instead of one request per file.
const treeEntries = [];
const utf8 = new TextDecoder("utf-8", { fatal: true });
for (const f of files) {
  const buf = readFileSync(path.join(ROOT, f));
  const isExec = (statSync(path.join(ROOT, f)).mode & 0o100) !== 0;
  const entry = { path: f, mode: isExec ? "100755" : "100644", type: "blob" };
  try {
    treeEntries.push({ ...entry, content: utf8.decode(buf) });
  } catch {
    const blob = await gh(api("/git/blobs"), {
      method: "POST",
      body: JSON.stringify({ content: buf.toString("base64"), encoding: "base64" }),
    });
    treeEntries.push({ ...entry, sha: blob.sha });
    await sleep(250);
  }
}
